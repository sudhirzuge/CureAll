...
const RelatedDoctors = ({ docId, speciality }) => {  // Fix: Destructure props correctly
  ...
  }, [doctors, speciality, docId]);  // Fix: Minimal dependencies
    ...
    <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
      ...
      <div className='w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
        {relDoc.slice(0, 5).map((item) => (
          <div 
            key={item._id}  // Fix: Use item._id as a key
